...
## 13-09-2026 · Cliente 13: «tus padres» NO se usa → «tus mayores» (Dirección)
**Qué pasó:** en la tanda de variaciones always on salieron titulares con «tus padres» («nosotros
cuidamos de tus padres», «¿Te preocupa cómo estarán tus padres…?», «Tus padres cuidados…»), copiando
el léxico de las piezas madre y del brief. Dirección: «la frase tus padres no se usa, elimínala y
reemplázala por tus mayores».
**Cómo se aplica:** en Cliente 13 el familiar atendido se nombra **«tus mayores»**, nunca «tus padres»,
en titular, apoyo, puntos y CTA. Se dice explícito al GPT en cada mensaje («no uses "tus padres": di
"tus mayores"») y el audit de copy lo busca en el píxel. Si la pieza madre lo lleva, la variación no lo
hereda.

## 13-09-2026 · Cliente 13: un ajuste en post que convierte el 9:16 en «1:1 incrustado» (Dirección)
**Qué pasó:** la VAR6 9:16 del GPT se salía de las zonas seguras y, como iba sobre crema liso, se
redujo TODO el bloque de contenido al 79 % y se centró entre y=290 e y=1520. Pasaba el medidor, pero
quedó un bloque encogido flotando con bandas vacías arriba, abajo y a los lados. Dirección: «es un 1:1
pegado en un 9:16». Es exactamente el fallo de §G.14, **provocado por el arreglo, no por el GPT**.
**Cómo se aplica:** reducir en bloque solo vale para recortes pequeños (≈90 % o más). Si hace falta
más, se **recompone**: texto repartido a lo ancho (107 px de lateral) en la mitad de arriba y la foto
**a sangre abajo**, que puede entrar en la franja inferior de 384 px porque no es texto. Y **después de
cualquier ajuste en post se vuelve a pasar §G.14 mirando la pieza**: que el medidor dé márgenes buenos
no dice nada de si parece un cuadrado pegado.

## 13-09-2026 · Cliente 13: aprobé la VAR6 9:16 «nativa» SIN mirar el marcado (Dirección: «¿cómo pudiste aprobar eso auditándolo?»)
**Qué pasó:** tras el «1:1 pegado», recompuse la pieza en post (texto arriba comprimido, foto de stock
original pegada abajo a sangre) y la aprobé mirando la imagen suelta. Estaba mal en todo: banda crema
vacía de 290 px arriba, logo rozando la franja superior, foto con corte duro y **la cabeza de la
cuidadora cortada**, laterales de la foto dentro del margen, y **ya no era la misma pieza que el 1:1**
(allí la foto va en recuadro redondeado y recreada por el GPT; aquí, a sangre y cruda).
**Cómo se aplica — obligatorio antes de aprobar CUALQUIER 9:16:**
1. Generar y **mirar** `zonas_seguras.py --marcar` (no solo leer el veredicto).
2. Ponerlo **al lado de su 1:1** y comprobar §A-quinquies: misma estructura, mismo tratamiento de foto.
3. Mirar caras y cabezas: nada cortado por un borde ni por la costura de un recuadro.
4. Si el GPT se sale de zonas, **se le vuelve a pedir con posiciones en píxeles por elemento**
   (logo y=…, titular y=…, sellos y=…, foto y=…); el post solo para desplazar o recortar poco.
   **Nunca montar una composición nueva en post con elementos sueltos**: no es un 9:16, es un collage.

## 13-09-2026 · Cliente 13: titular con doble lectura («Nóminas, altas y bajas…») (Dirección)
**Qué pasó:** se aprobó para la VAR6 el titular «Nóminas, altas y bajas gestionadas». Dirección: «se puede
malinterpretar porque parece que dice nóminas altas». Una enumeración sin artículos pone dos palabras
juntas que, leídas de corrido en 1,5 s de scroll, forman otra frase (y aquí además sugiere sueldos altos).
**Cómo se aplica:** en el audit de copy (§H), leer el titular **de corrido y sin comas**, como lo lee
el ojo en el feed, y buscar pares de palabras contiguas que cambien el sentido. Si aparece uno, se
reescribe con verbo o artículos («Nosotros gestionamos contrato y nóminas»), no se confía en la coma.

## 13-09-2026 · Cliente 13: `rclone copy --include "VAR*.png"` subió también las descartadas
**Qué pasó:** se subió la tanda al Drive con `rclone copy <Tanda N>/ ... --include "VAR*.png"`. El
patrón de rclone **entra en subcarpetas**, así que se subieron también `_work/_descartadas_*/` (piezas
con copy ya rechazado). Se detectó al listar el destino y se mandó `_work/` a la papelera del Drive.
**Cómo se aplica:** al subir, **`--max-depth 1`** (o `--include "/VAR*.png"` con la barra, que ancla a
la raíz) y **listar el destino después** (`rclone lsf -R`) para confirmar que solo están los finales y
el specs. Las carpetas de trabajo (`_work`, `_qc`) no salen nunca del disco local.

## 13-09-2026 · Cliente 13: `file_upload` no lee `~/Desktop/CLIENTES/`
**Qué pasó:** al re-adjuntar al GPT la pieza generada (guardada en `CLIENTES/<cliente>/Ads/GPT/Tanda N/`)
el `file_upload` de Claude-in-Chrome la rechazó: *«only files this session is allowed to read can be
uploaded»*. Los insumos que estaban en el scratchpad de la sesión sí subieron.
**Cómo se aplica:** todo lo que se vaya a adjuntar al GPT (brief `.txt`, logos, fotos, madres **y cada
pieza generada que se re-adjunta para un CAMBIOS o para sacar el 9:16**) se copia primero a una carpeta
`insumos/` dentro del scratchpad y se sube desde ahí. La carpeta del cliente es el destino final, no el
origen de las subidas.

## 14-09-2026 · Drive: no se sabía de qué madre era cada variación y las versiones se llamaban igual (Dirección)
**Qué pasó:** las piezas subían como `VAR1_…`, `VAR2_…`, `VAR6_…` con numeración corrida entre madres
(Cliente 13 Tanda 1: VAR1-2 de una madre, VAR3-4 de otra, VAR5-6 de otra), sin decir de cuál salía cada
una, y cada ronda de CAMBIOS se descargaba con el mismo nombre que la anterior (las descartadas acabaron
con nombres a mano tipo `VAR6_9x16_v7post.png`). En Drive era imposible saber qué era cada fichero.
**Cómo se aplica:** naming `Madre<m>_VAR<v>_<formato>_<angulo>_v<k>_<1x1|9x16>[_PEND].png` (`SKILL.md` §5).
`recoger_png.py` lo valida, no pisa un nombre existente y saca del registro las versiones anteriores;
`cerrar_tanda.py` sube solo la última versión de cada variación y avisa si falta el 1:1 o el 9:16.
