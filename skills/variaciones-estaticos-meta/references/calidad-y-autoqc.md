...
> **En variaciones importa el doble:** una variación rechazada por un fantasma se regenera, y cada
> regeneración la aleja de la pieza madre — que es justo lo que NO debe cambiar.
...
## Checks EXCLUSIVOS de las variaciones (11-09-2026)

Estos no están en `estaticos-meta`: solo aplican cuando se varía una pieza que ya existe. Van
**después** del §0 y del §G, no en su lugar. Son **cinco: V-1 a V-5** (no hay V-6, aunque algún
documento antiguo hablara de «V1-V6»).

### V-1 · ¿Es de verdad una variación? (prueba de la miniatura)
Pon la variación **al lado de la original y de las otras variaciones, a 200 px de ancho**. Si a
ese tamaño parecen la misma pieza, **NO es una variación**: Meta la lee igual y no abre público
nuevo. Se rehace. Se hace **siempre y con las tres juntas**, no de memoria.

### V-2 · ¿Respeta el ángulo? — RECHAZO si no
El ángulo, el formato, el avatar, la promesa y el CTA son intocables. Si cambió alguno, no es una
variación: es una pieza nueva y va por `estaticos-meta`.

### V-3 · ¿Hay una imagen REAL debajo? — RECHAZO si no
**Recrear sí, inventar no.** El GPT puede redibujar la persona o la escena **real que le
adjuntamos**; lo que no puede es inventar una cara que no existe ni un escenario de cero. Se
comprueba **de dónde salió cada imagen** y se anota en el specs: «Drive del cliente», «captura de su
web (fecha)» o «Pexels, id 1234567 (licencia)». **Sin fuente anotada, no se aprueba.** Y si la cara
generada no se parece a la referencia, el modelo inventó una persona → REGENERATE re-adjuntando la
foto.

### V-4 · ¿Se repite alguna imagen ENTRE VARIACIONES?
Dos variaciones con la misma foto no son dos variaciones. **Cero imágenes repetidas entre las
variaciones de una misma tanda.**
⚠️ **Esto NO aplica entre los dos ratios de la MISMA variación:** el 1:1 y el 9:16 de `VAR1` llevan
la misma foto a propósito (§A-quinquies exige que sea la misma foto reencuadrada, no regenerada).
El eje de este check es *entre piezas distintas*, nunca *entre tamaños de la misma pieza*.

### V-5 · ¿Cambió el fondo?
No basta con mover el texto. El fondo cambia en todas.
